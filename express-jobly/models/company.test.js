...
describe("findAll", function () {
  ...
  test("works: allfilters", async function () {
    let companies = await Company.findAll({
      name: "c",
      minEmployees: 1,
      maxEmployees: 2
  ...
  test("returns empty if no results", async function () {
    let companies = await Company.findAll({ name: "dsfsdfsdf" });
    expect(companies).toEqual([]);
  });

  test("throws error if minEmployees is larger than maxEmployees", async function () {
    try {
      let companies = await Company.findAll({
        minEmployees: 4,
        maxEmployees: 2
