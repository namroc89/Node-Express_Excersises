...
describe("GET /companies/ with filters", function () {
  ...
  test("works for all filters together", async function () {
    const resp = await request(app).get(
      `/companies/?maxEmployees=2&name=c&minEmployees=1`
    );
    expect(resp.statusCode).toBe(200);
    expect(resp.body).toEqual({
      companies: [
  ...
  test("returns empty list if none found", async function () {
    const resp = await request(app).get(`/companies/?name=124124124`);
    expect(resp.statusCode).toBe(200);
    expect(resp.body).toEqual({
      companies: []
    });
  ...
  test("returns error if min is greater than max", async function () {
    const resp = await request(app).get(
